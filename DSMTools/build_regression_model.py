...
from tools_data_access import TaskDataAccess
...
def build_XGBR(build_model_id, structure_data_file: str, predict_variable: str, categorical_variables: list[str]):
    ...
    try:
        ...
        # 对训练数据进行预测
        test_predictions = best_XGBR.predict(train_dataset)
...
def build_XGBRFR(build_model_id, structure_data_file: str, predict_variable: str, categorical_variables: list[str]):
    ...
    try:
        ...
        # 对训练数据进行预测
        train_predictions = best_XGBRFR.predict(train_dataset)
...
def build_SVR(build_model_id, structure_data_file: str, predict_variable: str, categorical_variables: list[str]) -> dict[
    ...
    try:
        ...
        # 对训练数据进行预测
        train_predictions = best_svr.predict(train_dataset)
...
将.csv中的所有数据作为数据集
'''
def prepare_dataset(structure_data_file:str, prop_name:str, categorical_features:list[str]):
    #导入数据
    ...
    train_dataset_sorted = train_dataset.sort_index(axis=1)
    return train_dataset_sorted, train_labels, categorical_features_info
...
def save_regressor_result(build_model_id:str, prop_name:str, algorithms:str, best_model, best_param,
    ...
    MSE = round(mean_squared_error(data_labels, data_predictions),4)
    RMSE = round(MSE ** 0.5, 4)
    R2 = round(r2_score(data_labels, data_predictions),4)
    print("均方误差 (MSE):", MSE)
    ...
    TaskDataAccess.set_build_model_finished(build_model_id, json.dumps(data)) # 保持到数据库,以供后续查询
