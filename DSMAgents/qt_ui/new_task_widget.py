from PyQt5.QtWidgets import QWidget,  QHBoxLayout, QLabel, QPushButton,QDialog
...
class AddTaskWidget(QWidget):
    """添加任务按钮（包含图标和文字）"""
    # 定义自定义信号(创建新任务)
    messageSent = pyqtSignal(TaskInfo)
    ...
    def __init__(self, parent=None):
        ...
        # 按钮
        self.push_button = QPushButton("新任务")
        ...
        self.push_button.setToolTip("创建一个新的土壤属性制图任务");
        self.push_button.clicked.connect(self.create_task)  # 绑定按钮点击事件
        layout.addWidget(self.push_button, alignment=Qt.AlignmentFlag.AlignCenter)

    def create_task(self):
        task_info = TaskInfo(self)
        task_info.task_id = str(uuid.uuid1())
        dialog = TaskDialog(task_info, self.parent)
