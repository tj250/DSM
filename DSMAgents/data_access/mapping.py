...
import config
from DSMAlgorithms import AlgorithmsType, DataTransformType
from DSMAlgorithms import (XGBRModel, RandomForestRegressionModel, SVRModel, KNRModel, PLSRModel,
                            GLMModel, MGWRModel, ElasticNetModel, CoKrigeModel, RegressionKrigeModel,
                            MLPModel, CustomModel, StackingModel)
...
def mapping(task_id: str, algorithms_id: str, mapping_area_file: str,
    ...
    algorithms_type, algorithms_info, model, mean_encoder, transform_info = BuildModelDataAccess.retrive_model(algorithms_id)
    if algorithms_type == AlgorithmsType.STACKING:
        ...
        prediction_model = CustomModel(model, transform_info, mean_encoder)

    if len(algorithms_info)>1:
        algorithms_name = '|'.join([item for item in algorithms_info.keys()])
    else:
        algorithms_name = next(iter(algorithms_info))

    # 是否需要坐标值作为预测的输入项
    need_coordinates_for_prediction = False
    if (algorithms_type == AlgorithmsType.STACKING or algorithms_type == AlgorithmsType.MGWR or
            algorithms_type == AlgorithmsType.RK or algorithms_type == AlgorithmsType.CK):  # 对于几种特定算法，需要坐标作为输入
    ...
    # if (algorithms_type == AlgorithmsType.EN or algorithms_type == AlgorithmsType.PLSR
    #         or algorithms_type == AlgorithmsType.RK or algorithms_type == AlgorithmsType.GLM
    #         or algorithms_type == AlgorithmsType.SVR or algorithms_type == AlgorithmsType.CUSTOM
    #         or (algorithms_type == AlgorithmsType.STACKING and (AlgorithmsType.EN.value in algorithms_name or
    #                                                             AlgorithmsType.PLSR.value in algorithms_name or
    #                                                             AlgorithmsType.RK.value in algorithms_name or
    #                                                             AlgorithmsType.GLM.value in algorithms_name or
    #                                                             AlgorithmsType.SVR.value in algorithms_name))):
    if hasattr(prediction_model.model, 'feature_names_in_'):  # 如果是内置单一模型（从sklearn接口派生的,具有feature_names_in_属性）
        model_features = prediction_model.model.feature_names_in_.tolist()
    elif isinstance(prediction_model.model, StackingRegressor): # 如果是堆叠模型
        model_features = prediction_model.model.estimators_[0].regressor.feature_names_in_.tolist()
    else:  # 如果是自定义回归器
        if hasattr(prediction_model.model, 'regressor'):
            model_features = prediction_model.model.regressor.feature_names_in_.tolist()
        else:
            model_features = BuildModelDataAccess.retrive_used_covariate(algorithms_id)
    columns = df_pixel_X_sorted.columns
    for col_name in columns:
        if (col_name != config.CSV_GEOM_COL_X and col_name != config.CSV_GEOM_COL_Y
                and (col_name not in model_features)): # col_name not in used_covariate and
    ...
    prediction_metrics.algorithms_name = algorithms_name
...
def create_uncertainty_map(task_id: str, algorithms_id: str, sample_file: str, mapping_area_file: str,
    ...
    print_caller_parameters()
    task_dir = os.path.join(DATA_OUTPUT_ROOT_DIR, task_id)
    # 构建预测模型。注意：预测模型必须采用经过重写的模型，而不能采用原始的回归器，重写的模型会在predict方法中，根据模型特性，对数据进行如下处理后再进行预测：
    # 1、平均编码，或xgboost的类别化
    # 2、z-score/min-max
    # 3、删除坐标列（针对无需geometry的算法）
    algorithms_type, algorithms_info, model, mean_encoder, transform_info = BuildModelDataAccess.retrive_model(algorithms_id)
    ...
    if find_cached_envs_arr:  # 存在匹配的缓存下来的X
        ...
        for i in range(len(build_model_params.interpretation_variables)):  # 遍历每个环境变量
            ...
            with open(env_cached_file, 'rb') as file:
                env_tif_data.append(pickle.load(file))
                print("读取环境变量缓存成功："+current_env_name)
        # 4、拼接缓存的环境变量有值数据，以像素为单位建立X（dataframe的每一行是一个像素）
        # 出于性能考虑，预先创建一个足够大的numpy数组，数组中的每一行存储需要进行预测的各个环境变量的属性值，即要参与预测的X
        print('开始拼接环境变量有值数据...')
        np_tiff_data = np.column_stack(tuple(env_tif_data))
    ...
    #         or (algorithms_type == AlgorithmsType.STACKING and (
    #         AlgorithmsType.EN.value in algorithms_name or AlgorithmsType.PLSR.value in algorithms_name
    #         or AlgorithmsType.RK.value in algorithms_name or AlgorithmsType.GLM.value in algorithms_name
    #         or AlgorithmsType.SVR.value in algorithms_name))):
    if hasattr(prediction_model.model, 'feature_names_in_'):  # 如果时单一模型（从sklearn接口派生的）
        model_features = prediction_model.model.feature_names_in_.tolist()
    elif isinstance(prediction_model.model, StackingRegressor): # 如果时堆叠模型
        model_features = prediction_model.model.estimators_[0].regressor.feature_names_in_.tolist()
    else:  # 如果是自定义回归器
        model_features = prediction_model.model.regressor.feature_names_in_.tolist()
    ...
    columns = df_pixel_X_sorted.columns
    for col_name in columns:
        if (col_name != config.CSV_GEOM_COL_X and col_name != config.CSV_GEOM_COL_Y and
                (col_name not in model_features)): # col_name not in used_covariate and
            df_pixel_X_sorted.pop(col_name)
    # 预测
    uncertainty_analysis = Uncertainty(prediction_model)  # 构建不确定性分析器
    # 准备训练的数据
    train_X, train_y, _, _, transform_info = prepare_model_train_dataset(False,
                                                            sample_file,
                                                            covariates_path,
                                                            42,
    ...
    # 需要根据zcore或normalize变换对原始环境变量进行变换
    tranform_type = transform_info["transform_type"]
    for vars, transform_params in transform_info.items():
        if vars in df_pixel_X_sorted.columns:
            if tranform_type == DataTransformType.Normalize.value:
                df_pixel_X_sorted[vars] = (df_pixel_X_sorted[vars] - transform_params['min']) / (transform_params['max'] - transform_params['min'] + 1e-8)
            elif tranform_type == DataTransformType.ZScore.value:
                df_pixel_X_sorted[vars] = (df_pixel_X_sorted[vars] - transform_params['mean']) / transform_params['std']
    ...
    evaluating_metrics.algorithms_name = algorithms_name
    ...
    if uncertainty_analysis.bootstrap_prediction_interval(train_X, train_y, df_pixel_X_sorted, bootstrap_times=5):
        ...
        for metric_type in uncertainty_metrics_type:
            # 确定一个唯一的文件名,作为不确定性图的文件名
            index = 0
            map_id = f"不确定性图_{metric_type.value}_{str(index)}"
            while os.path.exists(os.path.join(task_dir, map_id + ".tif")):
                index += 1
                map_id = f"不确定性图_{metric_type.value}_{str(index)}"
